...
parser$add_argument("--SCOREallele", type="character", help="PATH of SCORE  alleleinfo file")
parser$add_argument("--UMIallele", type="character", help="PATH of UMI  alleleinfo file")
parser$add_argument("--UMIandSCOREallele", type="character", help="PATH of UMIandSCORE alleleinfo file")
parser$add_argument("--moreInfo", type="character", help="PATH of moreInfo file")
parser$add_argument("--SCOREpairDist", type="character", help="PATH of SCORE  oneCell Node pair tree dist file")
parser$add_argument("--UMIpairDist", type="character", help="PATH of UMI  oneCell Node pair tree dist file")
parser$add_argument("--UMIandSCOREpairDist", type="character", help="PATH of UMIandSCORE oneCell Node pair tree dist file")
...
run_get_results <- function(alleleInfos_path, moreInfos, oneCellNode.pair.dist_path, checkString){
  ...
  haveANC.cells <- moreInfos %>% filter(., (!!sym(checkString)) == "anc") %>% `[`("BC") %>% unlist() %>% unique() # 188 cells
  haveANC.oneCellNode <- alleleInfos %>% filter(., BC %in% haveANC.cells & cellNum == 1) %>% `[`("NodeName") %>% unlist() # 151 oneCell node
  # check overlap infos
...
SCORE_result_list <- run_get_results(useArgs$SCOREallele, moreInfo, useArgs$SCOREpairDist, "bySCOREcheck")
UMI_result_list <- run_get_results(useArgs$UMIallele, moreInfo, useArgs$UMIpairDist, "byUMIcheck")
UMIandSCORE_result_list <- run_get_results(useArgs$UMIandSCOREallele, moreInfo, useArgs$UMIandSCOREpairDist, "byUMIandSCOREcheck")

all_results <- list(SCORE_result=SCORE_result_list,
                    UMI_result=UMI_result_list,
                    UMIandSCORE_result=UMIandSCORE_result_list)

saveRDS(all_results, useArgs$out)
